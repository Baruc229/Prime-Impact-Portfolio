...
const logoDark = fs.readFileSync(path.join(__dirname, '..', 'assets', 'pia-logo-dark-sm.png'));
const logoWhite = fs.readFileSync(path.join(__dirname, '..', 'assets', 'pia-logo-white-sm.png'));
const logoDarkB64 = logoDark.toString('base64');
const logoWhiteB64 = logoWhite.toString('base64');
...
<div class="report">
<div class="header"><img src="data:image/png;base64,${logoDarkB64}" alt="PIA"><h1>Rapport PIA</h1><p>${total} soumission${total > 1 ? 's' : ''} &middot; ${new Date().toLocaleDateString('fr-FR')}</p></div>
${sections}
</div></body></html>`;
}
